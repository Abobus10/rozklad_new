# -*- coding: utf-8 -*-
"""
Сервис уведомлений Telegram бота.

Современная архитектура с типизацией, валидацией и структурированным логированием.
Отвечает за отправку запланированных уведомлений пользователям.
"""
...
class NotificationService(LoggerMixin):
    """Сервис для управления уведомлениями бота."""
    
    def __init__(self):
        """Инициализация сервиса уведомлений."""
        self.timezone = pytz.timezone(config.timezone)
        self.logger.info("Инициализация сервиса уведомлений")
    ...
    async def handle_telegram_error(self, user_id: str, error: TelegramError, context: str) -> bool:
        ...
        Returns:
            True если пользователя нужно удалить из активных
        """
        error_msg = str(error)
        
        if isinstance(error, Forbidden):
            self.logger.warning(f"Пользователь {user_id} заблокировал бота: {error_msg}")
            # Деактивируем пользователя вместо удаления
            data_manager.update_user(user_id, {"active": False})
            return True
        elif "chat not found" in error_msg.lower():
            self.logger.warning(f"Чат {user_id} не найден: {error_msg}")
            return True
        else:
            self.logger.error(f"Ошибка отправки уведомления в {context} для {user_id}: {error_msg}")
            return False

    def _get_current_time(self) -> datetime:
        """Получает текущее время в нужной timezone."""
    ...
    def _get_tomorrow_info(self) -> Tuple[datetime, Optional[str]]:
        """
        Получает информацию о завтрашнем дне.
        
        Returns:
            Кортеж (дата_завтра, название_дня)
        """
    ...
    async def send_daily_reminders(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        """
        Отправляет ежедневные персональные напоминания о расписании на завтра.
        
        Запускается каждую минуту, проверяет настройки каждого пользователя.
        """
        current_time = self._format_time(self._get_current_time())
        tomorrow, day_name = self._get_tomorrow_info()
        
        self.logger.debug(f"Проверка ежедневных напоминаний на {current_time}")
        ...
        self.logger.info(f"Отправка ежедневных напоминаний {len(users_to_notify)} пользователям")
    ...
    def _get_users_for_daily_reminder(self, current_time: str) -> Dict[str, Any]:
        """
        Получает список пользователей для отправки ежедневных напоминаний.
        
        Args:
            current_time: Текущее время в формате HH:MM
            
        Returns:
            Словарь пользователей {user_id: user_data}
        """
        ...
        return {
            ...
            if (
                ...
                user_data.get("active", True)  # Проверяем, активен ли пользователь
            )
        }
    ...
    async def _send_daily_reminder_to_user(
        ...
        """
        Отправляет ежедневное напоминание одному пользователю.
        ...
        async with semaphore:
            try:
                ...
                if lessons:
                    ...
                    message_text = "🔔 *Напоминание*\n\nЗавтра пар нет! Можно отдыхать 😊"
                
                await self._send_simple_reminder(context, user_id, message_text)
                
            except Exception as e:
                self.logger.error(f"Ошибка отправки ежедневного напоминания пользователю {user_id}: {e}")

    async def _send_simple_reminder(self, context: ContextTypes.DEFAULT_TYPE, user_id: str, text: str) -> None:
        """
        Отправляет простое напоминание пользователю.
        ...
        try:
            ...
            # Планируем удаление сообщения через 12 часов
    ...
    async def send_morning_schedule(self, context: ContextTypes.DEFAULT_TYPE) -> None:
        """
        Отправляет утреннее расписание во все групповые чаты в 7:00.
        
        Перед отправкой нового сообщения открепляет и удаляет старое.
        """
        self.logger.info("Запуск утренней рассылки расписания для групповых чатов")
        ...
        if not day_name or today.weekday() == 6:
            self.logger.info(f"Сегодня {day_name or 'воскресенье'}, утреннее расписание не отправляется")
            return
        ...
        if not group_chats:
            self.logger.info("Нет зарегистрированных групповых чатов")
            return
        ...
        self.logger.info(f"Утреннее расписание отправлено в {success_count} из {len(group_chats)} чатов")
    ...
    async def _send_morning_schedule_to_chat(
        ...
        """
        Отправляет утреннее расписание в один групповой чат.
